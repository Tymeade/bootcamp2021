...
def normalize_pymorphy(text):
    tokens = re.findall(
        '[A-Za-zА-Яа-яЁё]+\-[A-Za-zА-Яа-яЁё]+|[A-Za-zА-Яа-яЁё]+', text)
    words = []
    for t in tokens:
        pv = morph.parse(t)
...
question_words = ['как', 'какой', 'где', 'что', 'кто', 'куда', 'откуда',
                  'когда', 'чем', 'сколько', 'чей']
...
stop_words_modified = ['и', 'в', 'во', 'не', 'что', 'он', 'на', 'я', 'с', 'со',
                       'как', 'а', 'то', 'все', 'она', 'так',
                       'его', 'но', 'да', 'ты', 'к', 'у', 'же', 'вы', 'за',
                       'бы', 'по', 'только', 'ее', 'мне', 'было',
                       'вот', 'от', 'меня', 'еще', 'нет', 'о', 'из', 'ему',
                       'теперь', 'когда', 'даже', 'ну', 'вдруг',
                       'ли', 'если', 'уже', 'или', 'ни', 'быть', 'был', 'него',
                       'до', 'вас', 'нибудь', 'опять', 'уж',
                       'вам', 'ведь', 'там', 'потом', 'себя', 'ничего', 'ей',
                       'может', 'они', 'тут', 'где', 'есть',
                       'надо', 'ней', 'для', 'мы', 'тебя', 'их', 'чем', 'была',
                       'сам', 'чтоб', 'без', 'будто', 'чего',
                       'раз', 'тоже', 'себе', 'под', 'будет', 'ж', 'тогда',
                       'кто', 'этот', 'того', 'потому', 'этого',
                       'какой', 'совсем', 'ним', 'здесь', 'этом', 'один',
                       'почти', 'мой', 'тем', 'чтобы', 'нее', 'сейчас',
                       'были', 'куда', 'зачем', 'всех', 'никогда', 'можно',
                       'при', 'наконец', 'два', 'об', 'другой', 'хоть',
                       'после', 'над', 'больше', 'тот', 'через', 'эти', 'нас',
                       'про', 'всего', 'них', 'какая', 'много',
                       'разве', 'три', 'эту', 'моя', 'впрочем', 'хорошо',
                       'свою', 'этой', 'перед', 'иногда', 'лучше',
                       'чуть', 'том', 'нельзя', 'такой', 'им', 'более',
                       'всегда', 'конечно', 'всю', 'между', 'который',
                       'свой', 'согласно', 'обычно', 'сколько', 'весь', 'часто',
                       'чей', 'всё', 'это', 'её', 'его',
                       'как', 'какой', 'где', 'что', 'кто', 'куда', 'откуда',
                       'когда', 'чем', 'сколько', 'чей', 'всё-таки', 'все-таки']
...
def preprocessing(questions):
    ...
    # questions['norm_wo_stop_words'] = questions['norm'].apply(
    ...
    # questions['adj'] = questions['norm_wo_stop_words'].apply(
    #     lambda x: normalize_pymystem_adjs(' '.join(x)))
    # questions['posl'] = questions['norm'].apply(lambda x: find_posl(x))
    # questions['citation'] = questions['Вопрос'].apply(find_citation)
    # questions['norm_wo_stop_words_mod'] = questions['norm'].apply(
    #     lambda x: remove_stop_words_mod(x))
    # questions['pogovorka'] = questions[['norm', 'Вопрос']].apply(find_pog_text,
    #                                                              axis=1)
    # questions['poslovica'] = questions[['norm', 'Вопрос']].apply(find_posl_text,
    #                                                              axis=1)
    # questions['pogovorki'] = (
    #             questions['pogovorka'].fillna('') + ' ' + questions[
    #         'poslovica'].fillna('')).str.strip(' ')
    # questions = questions.drop(columns=['pogovorka', 'poslovica'])
    # questions['names_types'] = questions['Вопрос'].apply(find_names)
    # questions['name_type'], questions['name'] = questions['names_types'].str[0], \
    #                                             questions['names_types'].str[1]
